...
const OrderCartMenu: React.FC<OrderCartProps> = ({ items, onPlaceOrder, onRemoveItem }) => {
      ...
      <div className="flex flex-col justify-between h-[calc(100%-2rem)] p-4">
        <div className="grid gap-2 max-h-[calc(100%-15rem)] overflow-auto">
          {items.map((food) => (
            <div key={food.id} className="border rounded-md h-16 flex gap-2 p-1 text-xs">
              ...
              <Button size="icon" variant="close" onClick={() => onRemoveItem(food.id)}>
                <X className="w-4 h-4" />
              </Button>
            </div>
          ))}
        ...
        <div className="p-2 mt-2">
          ...
          <div className="grid grid-cols-3 gap-3 m-5">
            ...
            <div className="border rounded-md grid place-content-center cursor-pointer h-14 text-black">
              <Smartphone />
            </div>
          </div>
